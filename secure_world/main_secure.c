...
int main(void) {
    ...
    // 2. TrustZone Configuration (SAU, IDAU) is often handled by bootrom or early startup,
    // but here we ensure specific peripherals/memory regions are configured if needed.
    // For RP2350, much of this is done via the Access Control Lists (ACLs) or SAU.
    // (Simulated for this skeleton)

    printf("Secure World: Preparing to jump to Non-Secure World at 0x%08X...\n", NON_SECURE_FLASH_OFFSET);

    // 3. Locate the Non-Secure Vector Table
    ...
    // 4. Set the Non-Secure Vector Table Offset Register (VTOR)
    // This tells the core where the NS interrupts are handled.
    // Note: We need to access the NS view of VTOR, or rely on the Core to bank it.
    // In Secure state, accessing 0xE000ED08 writes to the Secure VTOR. 
    // We need to write to the Non-Secure VTOR alias if available, or just rely on NS setup.
    // HOWEVER, strict CM33 transition requires setting up NS State Control.
    
    // For simplicity in this skeleton, we assume the CPU tracks NS VTOR separately
    // and we will set it via a specific register or let NS set its own VTOR in its startup (which is standard).
    // What IS critical is passing control.

    // 5. Jump to Non-Secure World
    // We use the CMSE intrinsic to perform the state transition.
    // `cmse_nsfptr_create` creates a function pointer with the LSB unset (indicating NS intent).
    // But typically we just cast the address. The critical part is using `__cmse_nonsecure_call` 
    // types if we were CALLING a function and returning.
    // For a one-way jump (reset), we can just branch to the address with LSB unset? 
    // Actually, LSB MUST be unset for NS destination in BLXNS.
