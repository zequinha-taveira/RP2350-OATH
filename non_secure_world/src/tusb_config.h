...
#define CFG_TUD_CONFIG_DESC_LEN                                                \
  (TUD_CONFIG_DESC_LEN + TUD_CDC_DESC_LEN + TUD_MSC_DESC_LEN +                 \
   TUD_HID_DESC_LEN + TUD_CCID_DESC_LEN)

// Number of Endpoints required
#define CFG_TUD_ENDPOINT_MAX 4

//------------- CCID -------------//
// Custom CCID class (Smart Card Reader) is required for Yubico Authenticator
// compatibility. Class: 0x0B (Smart Card)
...
// Vendor ID and Product ID (Yubico)
#define CFG_TUD_VID 0x1050
#define CFG_TUD_PID 0x0407
