...
bool bio_enroll_start(uint8_t modality, uint8_t *name, uint8_t name_len, bio_enroll_result_t *result) {
    ...
    memcpy(bio_state.template_name, name, name_len > MAX_BIO_NAME_LEN ? MAX_BIO_NAME_LEN : name_len);
...
bool bio_enroll_process_sample(uint8_t *sample, uint8_t sample_len, bio_enroll_result_t *result) {
    ...
    if (quality < bio_state.quality_threshold) {
        ...
        result->remaining_samples = bio_state.samples_required - bio_state.samples_collected;
        return true;
    }
...
bool bio_enroll_set_name(uint8_t *template_id, uint8_t *name, uint8_t name_len) {
    for (int i = 0; i < fingerprint_count; i++) {
        if (memcmp(fingerprint_templates[i].template_id, template_id, 32) == 0) {
            ...
            memcpy(fingerprint_templates[i].name, name, name_len > MAX_BIO_NAME_LEN ? MAX_BIO_NAME_LEN : name_len);
    ...
    for (int i = 0; i < fingerprint_count; i++) {
        if (memcmp(fingerprint_templates[i].template_id, template_id, 32) == 0) {
            ...
            for (int j = i; j < fingerprint_count - 1; j++) {
                memcpy(&fingerprint_templates[j], &fingerprint_templates[j + 1], sizeof(fingerprint_template_t));
            }
            fingerprint_count--;
            memset(&fingerprint_templates[fingerprint_count], 0, sizeof(fingerprint_template_t));
...
bool bio_enumerate_credentials(bio_credential_info_t *credentials, uint8_t *count) {
    *count = fingerprint_count;
    ...
    for (int i = 0; i < fingerprint_count; i++) {
        memcpy(credentials[i].template_id, fingerprint_templates[i].template_id, 32);
        credentials[i].name_len = MAX_BIO_NAME_LEN;
        memcpy(credentials[i].name, fingerprint_templates[i].name, MAX_BIO_NAME_LEN);
...
bool fingerprint_enroll_start(uint8_t *name, uint8_t name_len) {
    return bio_enroll_start(MODALITY_FINGERPRINT, name, name_len, &(bio_enroll_result_t){0});
}
...
bool store_fingerprint_template(fingerprint_template_t *templ) {
    ...
    memcpy(&fingerprint_templates[fingerprint_count], templ, sizeof(fingerprint_template_t));
    fingerprint_count++;
...
uint8_t bio_sensor_get_quality(void) {
    ...
    if (quality < 60) quality = 85; // Keep quality reasonable
