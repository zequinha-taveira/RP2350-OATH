...
static ctap21_state_t ctap21_state = {.options = {.rk = true,
                                                  .uv = false,
                                                  .up = true,
                                                  .plat = false,
                                                  .client_pin = false,
...
bool ctap21_get_credentials(uint8_t *rp_id, uint8_t rp_len,
  ...
  for (int i = 0; i < ctap21_state.credential_count && count < 10; i++) {
    if (memcmp(ctap21_state.resident_keys[i].rp_id, rp_id, rp_len) == 0) {
      memcpy(&resp->credentials[count], &ctap21_state.resident_keys[i],
             sizeof(resident_key_t));
      count++;
    }
  }
...
bool ctap21_delete_credential(uint8_t *credential_id) {
  ...
  for (int i = 0; i < ctap21_state.credential_count; i++) {
    if (memcmp(ctap21_state.resident_keys[i].credential_id, credential_id,
               32) == 0) {
      // Remove by shifting
      for (int j = i; j < ctap21_state.credential_count - 1; j++) {
        memcpy(&ctap21_state.resident_keys[j],
               &ctap21_state.resident_keys[j + 1], sizeof(resident_key_t));
      }
      ctap21_state.credential_count--;
      memset(&ctap21_state.resident_keys[ctap21_state.credential_count], 0,
             sizeof(resident_key_t));
      return true;
    }
  }
...
bool ctap21_update_user(uint8_t *credential_id, uint8_t *user_name,
  ...
  for (int i = 0; i < ctap21_state.credential_count; i++) {
    if (memcmp(ctap21_state.resident_keys[i].credential_id, credential_id,
      ...
      memcpy(ctap21_state.resident_keys[i].user_name, user_name,
             name_len > 64 ? 64 : name_len);
      return true;
    }
  }
  return false;
...
bool ctap21_bio_get_info(ctap21_bio_info_t *info) {
  ...
  info->capabilities =
      CAPABILITY_TIMEOUT | CAPABILITY_USER_VER | CAPABILITY_ENROLL;
...
bool ctap21_store_resident_key(resident_key_t *rk) {
  ...
  memcpy(&ctap21_state.resident_keys[ctap21_state.credential_count], rk,
         sizeof(resident_key_t));
  ctap21_state.credential_count++;
...
bool ctap21_list_resident_keys(credential_mgmt_resp_t *resp) {
  ...
  memcpy(resp->credentials, ctap21_state.resident_keys,
         sizeof(resident_key_t) * ctap21_state.credential_count);
  resp->status = 0x00;
  return true;
}
...
bool ctap21_check_always_uv(void) { return ctap21_state.always_uv; }
