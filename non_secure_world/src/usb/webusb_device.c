...
static webusb_state_t webusb_state = {
    .is_connected = false, .is_configured = false, .ep_out = 0, .ep_in = 0};
...
static const uint8_t webusb_platform_descriptor[] = {
    // Length (2 bytes)
    WEBUSB_PLATFORM_DESCRIPTOR_LENGTH & 0xFF,
    (WEBUSB_PLATFORM_DESCRIPTOR_LENGTH >> 8) & 0xFF,
...
uint16_t webusb_get_descriptor(uint8_t *buffer, uint16_t bufsize) {
  ...
  memcpy(buffer, webusb_platform_descriptor,
         sizeof(webusb_platform_descriptor));
...
static bool webusb_user_control_xfer_cb(uint8_t rhport, uint8_t stage,
  ...
  case WEBUSB_REQ_GET_URL:
    if (stage == CONTROL_STAGE_SETUP) {
      ...
      return tud_control_xfer(rhport, request, (void *)webusb_url_descriptor,
                              len);
    }
    break;
...
void webusb_handle_command(uint8_t const *msg, uint32_t len) {
  ...
  case WEBUSB_CMD_HSM_GEN_KEY:
    if (len >= 2) {
      uint8_t slot = msg[1];
      uint8_t status = 0;
      if (secure_gateway_hsm_gen_key(slot, &status)) {
        response[0] = WEBUSB_CMD_HSM_GEN_KEY;
        response[1] = status;
        response_len = 2;
      } else {
        response[0] = WEBUSB_CMD_HSM_GEN_KEY;
  ...
  case WEBUSB_CMD_HSM_GET_PUBKEY:
    if (len >= 2) {
      uint8_t slot = msg[1];
      uint16_t pub_len = 0;
      if (secure_gateway_hsm_get_pubkey(slot, response + 2, &pub_len)) {
        response[0] = WEBUSB_CMD_HSM_GET_PUBKEY;
        response[1] = WEBUSB_STATUS_OK;
        response_len = 2 + pub_len;
      } else {
        response[0] = WEBUSB_CMD_HSM_GET_PUBKEY;
        response[1] = WEBUSB_STATUS_ERROR;
        response_len = 2;
  ...
  case WEBUSB_CMD_HSM_SIGN:
    if (len >= 34) { // 1 (cmd) + 1 (slot) + 32 (hash)
      uint8_t slot = msg[1];
      uint16_t sig_len = 0;
      if (secure_gateway_hsm_sign(slot, msg + 2, response + 2, &sig_len)) {
        response[0] = WEBUSB_CMD_HSM_SIGN;
        response[1] = WEBUSB_STATUS_OK;
        response_len = 2 + sig_len;
      } else {
        response[0] = WEBUSB_CMD_HSM_SIGN;
        response[1] = WEBUSB_STATUS_ERROR;
        response_len = 2;
